...
class OauthClient:
    ...
    async def async_get_jwt_url(self):
        return 'http://{}/oauth/token'.format(await self.consul_client.async_resolve_service(self.uaa_name))

    async def async_get_public_key_url(self):
        return 'http://{}/oauth/token_key'.format(await self.consul_client.async_resolve_service(self.uaa_name))
