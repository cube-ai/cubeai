...
class DeploymentApiA(tornado.web.RequestHandler):
    ...
    async def post(self, *args, **kwargs):
        ...
        await deployment_db.create_deployment(deployment)
        self.set_status(201)
        self.finish()
    ...
    async def get(self, *args, **kwargs):
        ...
        if where != '':
            where = 'WHERE ' + where
        total_count, result = await deployment_db.get_deployments(where, pageable)
        self.set_header('X-Total-Count', total_count)

        self.write(json.dumps(result))


class DeploymentApiB(tornado.web.RequestHandler):
    ...
    async def put(self, attr, *args, **kwargs):
        ...
        deployment = Deployment()
        deployment.__dict__ = await deployment_db.get_deployment_by_id(body.get('id'))

        if attr == 'solutioninfo':
            solution_uuld = deployment.solutionUuid
            solutions = await solution_db.get_solutions_by_uuid(solution_uuld)
            solution = solutions[0]

            deployment.solutionName = solution.get('name')
            deployment.pictureUrl = solution.get('pictureUrl')

            await deployment_db.update_deployment_solutioninfo(deployment)
            self.set_status(201)
            self.finish()
            ...
            await deployment_db.update_deployment_admininfo(deployment)
            self.set_status(201)
            self.finish()
            ...
            await deployment_db.update_deployment_demourl(deployment)
            self.set_status(201)
            self.finish()
            ...
            await deployment_db.update_deployment_status(deployment)
            self.set_status(201)
            self.finish()
            ...
            await deployment_db.update_deployment_star_count(deployment)
            self.set_status(201)
            self.finish()
    ...
    async def delete(self, id, *args, **kwargs):
        ...
        await deployment_db.delete_deployment(id)
